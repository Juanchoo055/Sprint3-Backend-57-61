...
module.exports = {
    ...
    update : async(req, res, next) => {
        try {
            const reg = await Articulo.update({categoriaid: req.body.categoriaid, codigo: req.body.codigo, nombre: req.body.nombre, descripcion: req.body.descripcion},{ where: { id: req.body.id } });
            res.status(200).json(reg);
        } catch (e) {
            res.status(500).send({
                message: 'Ocurrió un error'
